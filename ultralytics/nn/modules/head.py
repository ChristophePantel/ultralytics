...
class Detect(nn.Module):
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            preds (torch.Tensor): Raw predictions with shape (batch_size, num_anchors, 4 + 2 * nc + extra) with last 
                dimension format [x1, y1, x2, y2, class_probs, km_probs, extra], where extra holds the mask coefficients,
    ...
    # Segment, Pose and OBB carry task channels after the class scores, Detect has none
        if self.use_km_scores:
            boxes, scores, km_scores, *extra = preds.split([s for s in (4, self.nc, self.nc, preds.shape[-1] - 4 - 2 * self.nc) if s], dim=-1) # (CP/IRIT): Add predicted knowledge model scores
        else:
